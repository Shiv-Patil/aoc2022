class Monkey:
    ...
    def _set_test(self, string: str, true: int, false: int, divisors: "list[int]"):
        divisor = int(string.split()[2])
        divisors.append(divisor)
        self.test = lambda x: true if x % divisor == 0 else false

    def play_turn(self, monkeys: "list[Monkey]", congruence_modulo: int):
        while len(self.items):
            item = self.items.pop(0)
            item = self.inspect(item) % congruence_modulo
...
while inp := input():
    ...
    monkeys.append(
        Monkey(
            starting_items,
            operation_string,
            (test_string, test_true, test_false),
            divisors,
        )
    )
...
for divisor in divisors:
    congruence_modulo *= divisor
for round in range(rounds):
    for monkey in monkeys:
        monkey.play_turn(monkeys, congruence_modulo)
